...
export function calcRewards({ mode, kills, deaths, won, wavesCleared, diff, isTraining }) {
  ...
  if (kills > 0)         addR(`${t('rwKills')} ×${kills}`,         kills * 85,          kills * 95);
  if (won)               addR(t('rwVictory'),                        350,                 320);
  if (wavesCleared > 0)  addR(`${t('rwWaves')} ×${wavesCleared}`,   wavesCleared * 140,  wavesCleared * 130);
